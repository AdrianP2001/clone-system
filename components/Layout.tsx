...
/* const Layout: React.FC<LayoutProps> = ({
  ...
  const menuItems = [
    ...
    { id: 'invoices', label: 'Emisión', icon: '📄' },
    { id: 'credit-notes', label: 'Notas de Crédito', icon: '🔄' },
    { id: 'retentions', label: 'Retenciones', icon: '💰' },
    { id: 'remittances', label: 'Guías de Remisión', icon: '🚚' },
    { id: 'settlements', label: 'Liquidaciones', icon: '�' },
    { id: 'reports', label: 'Reportes y SRI', icon: '📈' },
    { id: 'sales-book', label: 'Libro de Ventas', icon: '📚' },
    { id: 'ats', label: 'ATS', icon: '📄' },
    { id: 'form-104', label: 'Formulario 104', icon: '💰' },
    { id: 'kardex', label: 'Kardex', icon: '📦' },
    { id: 'profitability', label: 'Rentabilidad', icon: '📈' },
    { id: 'notifications', label: 'Notificaciones', icon: '📧' },
    { id: 'clients', label: 'Entidades', icon: '👥' },
    { id: 'products', label: 'Inventario', icon: '🏷️' },
    { id: 'integrations', label: 'Integración Web', icon: '🔌' },
    { id: 'config', label: 'Perfil de Empresa', icon: '🏢' },
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center space-x-3 px-4 py-3.5 rounded-2xl transition-all ${activeTab === item.id
          ...
          <div className="flex items-center space-x-3 bg-slate-800/40 p-3 rounded-2xl border border-slate-700/50">
            <div className="w-9 h-9 rounded-xl bg-slate-700 flex items-center justify-center font-black text-white shadow-lg overflow-hidden border border-slate-600">
              {businessInfo.logo ? (
                ...
                <span>{businessInfo.name.charAt(0)}</span>
              )}
            </div>
...
const Layout: React.FC<LayoutProps> = ({
  ...
  const menuItems = [
    ...
    { id: 'config', label: 'Perfil de Empresa', icon: '🏢' },
    { id: 'ai-assistant', label: 'Asistente IA', icon: '🤖' },
    // Este ítem activará el logout
    { id: 'logout_btn', label: 'Cerrar Sesión', icon: '🚪' },
  ];
